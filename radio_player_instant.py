...
class RadioPlayer:
    def __init__(self, root):
        self.root = root
        self.root.title("🎵 Lecteur Radio - Reprise Instantanée en Direct")
    ...
    def setup_ui(self):
        ...
        # Titre
        title_label = ttk.Label(main_frame, text="🎵 Lecteur Radio", font=("Arial", 16, "bold"))
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 20))
        
        # Sélection de la radio
        ttk.Label(main_frame, text="Radio:").grid(row=1, column=0, sticky=tk.W, pady=5)
        ...
        self.station_combo.grid(row=1, column=1, sticky=(tk.W, tk.E), pady=5, padx=(10, 0))
        ...
        self.play_button = ttk.Button(button_frame, text="▶️ Play", command=self.toggle_play_pause, width=15)
        self.play_button.grid(row=0, column=0, padx=5)
        
        self.stop_button = ttk.Button(button_frame, text="⏹️ Stop", command=self.stop_radio, width=15)
        self.stop_button.grid(row=0, column=1, padx=5)
        
        self.history_button = ttk.Button(button_frame, text="📋 Historique", command=self.show_history, width=15)
        ...
        metadata_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10)
        ...
        url_frame.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10)
   ...
   → La radio reprend INSTANTANÉMENT en DIRECT
   ...
   → Toujours en temps réel comme une vraie radio !"""
    ...
    def toggle_play_pause(self):
        ...
        if self.is_playing:
            ...
            print(f"▶️  Reprise en direct: {self.current_station}")
                    ...
                    if self.is_playing and self.current_station and self.current_url:
                        ...
                        if metadata:
                            ...
                            if self.last_metadata != (metadata.artist, metadata.title):
                                print(f"🎵 [{self.current_station}] {metadata.artist} - {metadata.title}")
                                self.last_metadata = (metadata.artist, metadata.title)
                    
                    time.sleep(5)  # Vérifier toutes les 5 secondes
                    
                except Exception as e:
                    print(f"Erreur de métadonnées: {e}")
                    time.sleep(10)
